...
function endsWith(str, searchString) {
  ...
  let index = str_.length - searchString.length
...
*/
let externalGlobalsObj = externalGlobals({
  /*
...
export default defineConfig(function (_a) {
  ...
  let root = process.cwd() // /Users/zhangqingchen/Desktop/前端Pro/React/react-vite
  /* 1. 获取src目录 */
  let pathSrc = path.resolve(__dirname, 'src') // /Users/zhangqingchen/Desktop/前端Pro/React/react-vite/src
  /* 2. 获取src目录 */
  let baseSrc = fileURLToPath(new URL('./src', import.meta.url)) // /Users/zhangqingchen/Desktop/前端Pro/React/react-vite/src
      ...
      preprocessorOptions: {
        ...
        scss: {
          ...
          /*
                     ...
                     */
          additionalData: '\n          @use "'
            .concat(path.resolve(__dirname, 'src/styles/theme/scss-theme.scss'), '" as *;\n          @use "')
            .concat(path.resolve(__dirname, 'src/styles/scss/global-auto-variable.scss'), '" as *;\n          '),
        ...
        output: {
          ...
          assetFileNames: function (assetInfo) {
            if (assetInfo.names && assetInfo.names.length) {
              ...
              if (
                ['.png', '.jpg', '.jpeg', '.webp', '.svg', '.gif'].some(function (ext) {
                  return endsWith(assetInfo.names, ext)
                })
              ) {
          ...
          manualChunks: function (id) {
            ...
            if (id.includes('/views/')) {
              let moduleName = id.split('/views/')[1].split('/')[0]
      ...
      proxy:
        ((_b = {
          ...
          '/api/downloadVideo': {
            ...
            changeOrigin: true,
            rewrite: function (path) {
              return path.replace(/^\/api\/downloadVideo/, '')
            }
          },
          ...
          '/api/downloadPdf': {
            ...
            changeOrigin: true,
            rewrite: function (path) {
              return path.replace(/^\/api\/downloadPdf/, '')
            }
          },
          ...
          '/api/download': {
            ...
            changeOrigin: true,
            rewrite: function (path) {
              return path.replace(/^\/api\/download/, '')
            }
          },
          /* 代理跨域(外地服务器-图片下载) */
          '/api/images': {
            ...
            changeOrigin: true,
            rewrite: function (path) {
              return path.replace(/^\/api\/images/, '')
            }
          },
          /* 代理跨域(外地服务器-卖座电影列表) */
          '/api/maizuo': {
            ...
            changeOrigin: true,
            rewrite: function (path) {
              return path.replace(/^\/api\/maizuo/, '')
            }
          }
        }),
        /* 代理跨域 (本地服务器 Apifox) */
